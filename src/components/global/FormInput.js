...
function FormInput(props) {
  ...
  // Activate and Deactivate the form label
  useEffect(() => {
    const inputField = inputRef.current;
    const correspondingLabelField = inputField.parentElement.lastElementChild;

    function activateLabel() {
      correspondingLabelField.setAttribute("class", "label-material active");
    }

    function deactivateLabel() {
      correspondingLabelField.setAttribute("class", "label-material");
    }

    // Activate label for prefilled values
    if (inputField.value !== "") activateLabel();

    // Move label on focus
    inputField.addEventListener("focus", () => activateLabel());
